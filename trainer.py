...
def process_image(info, resize_size_y=200):
    ...
    for img in info:
        ...
        if len(face_location) > 1 or len(face_location) == 0:
            print("cannot contain more than one face."
                  f"skipping..")
            return
        ...
        face_encoding = face_recognition.face_encodings(resize_by_height(deepcopy(img[top:bottom, left:right]), resize_size_y))
        if face_encoding:
...
if __name__ == "__main__":
    ...
    print(tabulate.tabulate(list(zip(range(len(files)), map(lambda x: noMn(path.basename(x)), files),
                                     map(lambda x: f"{round(path.getsize(x) / 1080)} kb", files))),
                            headers=['No.', 'Filenames', "Size"], tablefmt='fancy_grid'))
