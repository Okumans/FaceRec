...
find_distance: Callable[[int, int, float, float, float], float] = lambda W, H, res, fw, fh: (
    ((H * res) * (W * res)) / 518400
) * (((fw + fh) / 2) / 220.39) ** (1 / -0.949)
            ...
            if degree_raw == "0xy degree":
                ...
                if box:
                    ...
                    if width * height >= 0:
                        ...
                        image = cv2.putText(
                            image, f"{round(dist, 2)}M {index}", (0, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2
                        )
