...
if __name__ == "__main__":
    ...
    for folder in sorted(folders, key=lambda a: int(path.basename(a).split("_")[0])):
        ...
        for degree_folder in glob.glob(f"{folder}/*"):
            axis: str = "".join(
                ["" if i.isnumeric() or i == "-" else i for i in path.basename(degree_folder).split()[0]]
            )
            degree: str = "".join(
                [i if i.isnumeric() or i == "-" else "" for i in path.basename(degree_folder).split()[0]]
            )
    ...
    for angle_folder in images_by_angle:
        ...
        for image in images_by_angle[angle_folder]:
            ...
            cv2.imshow(
                ...
                cv2.putText(
                    image,
                    angle_folder[1] + angle_folder[0] + " " + str(counter),
                    (0, 20),
            ...
            cv2.waitKey(1)
        print(
            f"{angle_counter_success}/{angle_counter_all} ({-1 if angle_counter_all == 0 else angle_counter_success/angle_counter_all*100})\n"
        )
