...
class Detection:
    def __init__(self):
        caffemodel = path.dirname(__file__) + r"/detection_model/Widerface-RetinaFace.caffemodel"
        deploy = path.dirname(__file__) + r"/detection_model/deploy.prototxt"
...
class AntiSpoofPredict(Detection):
    def __init__(self, device_id):
        ...
        self.onnx_session_1 = onnxruntime.InferenceSession(path.dirname(__file__) + r"/onnx_models/Anti-Spoof_078.onnx")
        self.onnx_session_2 = onnxruntime.InferenceSession(path.dirname(__file__) + r"/onnx_models/Anti-Spoof_943.onnx")
    ...
    def predict_onnx(self, img, model_no):
        ...
        result = F.softmax(test_transform(ort_outs[0]).squeeze(), dim=-1).cpu().numpy()
        return result
